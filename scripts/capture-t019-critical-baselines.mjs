...
const outDir = path.resolve("docs/evidence/TB-P05-T019");
...
async function withSession(viewport, work) {
  ...
  const userData = fs.mkdtempSync(path.join(process.env.TEMP || ".", "tooba-t019-chrome-"));
...
async function main() {
  await withSession({ width: 1440, height: 900, mobile: false }, async ({ send }) => {
    ...
    await shot(send, "06-home-desktop-baseline.png", true);
  });
  await withSession({ width: 390, height: 844, mobile: true }, async ({ send }) => {
    await navigate(send, `${tooba}/`);
    await shot(send, "07-home-mobile-baseline.png", true);
  });
  await withSession({ width: 1440, height: 900, mobile: false }, async ({ send }) => {
    await navigate(send, toobaPdp);
    await shot(send, "08-pdp-desktop-baseline.png", true);
    ...
    await shot(send, "09-pdp-tabs-baseline.png");
  });
  await withSession({ width: 390, height: 844, mobile: true }, async ({ send }) => {
    await navigate(send, toobaPdp);
    await shot(send, "10-pdp-mobile-baseline.png", true);
  });
  console.log("t019 critical baselines done");
}
