...
Last Implementation Task: TB-P06-T028-R1
...
Evidence for current task: `docs/evidence/TB-P06-T028-R1/` (R1 completed concrete User-Preview URLs + browser proof; financial design unchanged; mixed tender still deferred; Shopeiva-locked UI). Flags: `WALLET_CHECKOUT_FULL_PAYMENT` = LIVE; `REFUND_TO_WALLET` = LIVE; `WALLET_CHECKOUT_BROWSER_PROOF` = LIVE; `REFUND_TO_WALLET_BROWSER_PROOF` = LIVE; `WALLET_CHECKOUT_USER_PREVIEW` = READY; `WALLET_MIXED_TENDER` = DEFERRED; `VISUAL_CONTRACT` = SHOPEIVA_LOCKED. Do **not** claim `WALLET_MIXED_TENDER_LIVE`, `USER_VISUAL_ACCEPTED`, `PRODUCT_FULLY_READY`, or `PRODUCTION_GO_LIVE_READY`.
