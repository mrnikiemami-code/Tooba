...
P05 Operational Surface Integration = IN_PROGRESS (TB-P05-T001 REPAIR / AWAITING_ARCHITECT_ACCEPT)
```
...
6. TB-P04-GATE is Architect-accepted. P04 = COMPLETE. P05 = IN_PROGRESS. Current envelope result is TB-P05-T001 REPAIR AWAITING_ARCHITECT_ACCEPT. Do not invent TB-P05-T002. Seller authorization binds authenticated actor → Party membership → SpiceDB/InMemory `party#view`; SellerPartyId is context only. Purchased Shopeiva is Next 16.2.6 / React 19.2.4 / Tailwind 4. Sell-first rule: preserve Shopeiva with minimum change and connect Tooba live APIs. Seller repair evidence: `docs/evidence/TB-P05-T001/repair/`. Deferred: Payment missing IdempotencyKey → 500/NRE; Cart replace-hold release→reserve window.
