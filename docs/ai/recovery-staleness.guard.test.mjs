/**
 * Recovery SoT staleness guard (TB-P09-T008).
...
const CURRENT_TASK_ID = "TB-P09-T008";
...
test("recovery SoT points Last Implementation at TB-P09-T008; Architect T007; Issued none; Repair none", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);

  assert.match(recovery, /TB-P09-T007/);
  assert.match(state, /TB-P09-T007/);
  assert.match(recovery, /P09/);
  assert.match(state, /P09/);

  assert.match(
    state,
    /Last Architect Accepted Task:\s*```text\s*TB-P09-T007\s*```/,
    "PROJECT-STATE Last Architect Accepted Task must be TB-P09-T007",
  );
  assert.match(
    state,
    /Last Architect-Accepted Task:\s*```text\s*TB-P09-T007\s*```/,
  ...
  assert.match(
    recovery,
    /Last Architect Accepted Task:\s*```text\s*TB-P09-T007\s*```/,
    "recovery Last Architect Accepted Task must be TB-P09-T007",
  );
  assert.match(
    state,
    /Last Implementation Task:\s*```text\s*TB-P09-T008\s*```/,
    "PROJECT-STATE Last Implementation Task must be TB-P09-T008",
  );
  assert.match(
    recovery,
    /Last Implementation Task:\s*```text\s*TB-P09-T008\s*```/,
    "recovery Last Implementation Task must be TB-P09-T008",
  );
  assert.match(
    state,
  ...
  assert.equal(CURRENT_TASK_ID, "TB-P09-T008");
  assert.doesNotMatch(state, /TB-P09-T009/);
  assert.doesNotMatch(recovery, /TB-P09-T009/);
});
...
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(implRecovery, "TB-P09-T008");
  assert.equal(implState, "TB-P09-T008");
