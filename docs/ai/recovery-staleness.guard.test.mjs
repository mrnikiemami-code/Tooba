...
const CURRENT_TASK_ID = "TB-P10-T004-R7";
const REQUIRED_MARKERS = ["P10","TB-P10-T004-R5","TB-P10-T004-R6","TB-P10-T004-R7","USER_VISUAL_ACCEPTED","BRIDGE-WAKE-V1"];
const STALE = ["TB-P10-T004","TB-P10-T004-R1","TB-P10-T004-R4","TB-P10-T004-R5","TB-P10-T004-R6"];
...
test("recovery SoT points Architect at TB-P10-T004-R6; Impl TB-P10-T004-R7; Phase P10", () => {
  const recovery = read(recoveryPath); const state = read(statePath);
  assert.match(state, /Last Architect Accepted Task:\s*```text\s*TB-P10-T004-R6\s*```/);
  assert.match(recovery, /Last Architect Accepted Task:\s*```text\s*TB-P10-T004-R6\s*```/);
  assert.match(state, /Last Implementation Task:\s*```text\s*TB-P10-T004-R7\s*```/);
  assert.match(recovery, /Last Implementation Task:\s*```text\s*TB-P10-T004-R7\s*```/);
  ...
  assert.equal(CURRENT_TASK_ID, "TB-P10-T004-R7");
});
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(a, "TB-P10-T004-R7"); assert.equal(b, "TB-P10-T004-R7");
