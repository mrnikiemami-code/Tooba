/**
 * Recovery SoT staleness guard (TB-P10-T004-R1).
...
const CURRENT_TASK_ID = "TB-P10-T004-R4";
...
test("recovery SoT points Architect at TB-P10-T004-R3; Impl TB-P10-T004-R4; Phase P10", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);
  assert.match(recovery, /P10/);
  assert.match(state, /P10/);
  assert.match(state, /Last Architect Accepted Task:\s*```text\s*TB-P10-T004-R3\s*```/);
  assert.match(state, /Last Architect-Accepted Task:\s*```text\s*TB-P10-T004-R3\s*```/);
  assert.match(recovery, /Last Architect Accepted Task:\s*```text\s*TB-P10-T004-R3\s*```/);
  assert.match(state, /Last Implementation Task:\s*```text\s*TB-P10-T004-R4\s*```/);
  assert.match(recovery, /Last Implementation Task:\s*```text\s*TB-P10-T004-R4\s*```/);
  ...
  assert.equal(CURRENT_TASK_ID, "TB-P10-T004-R4");
});
...
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(implRecovery, "TB-P10-T004-R4");
  assert.equal(implState, "TB-P10-T004-R4");
