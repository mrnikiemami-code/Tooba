/**
 * Recovery SoT staleness guard (TB-P07-T039).
...
const CURRENT_TASK_ID = "TB-P07-T039";
...
test("recovery SoT current issued/repair points at TB-P07-T039 not stale pointers", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);
  ...
  assert.match(
    state,
    /Current Issued Task:\s*```text\s*TB-P07-T039\s*```/,
    "PROJECT-STATE Current Issued Task must be TB-P07-T039",
  );
