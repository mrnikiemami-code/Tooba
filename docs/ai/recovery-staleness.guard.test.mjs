...
const CURRENT_TASK_ID = "TB-P10-T004-R1";
...
test("recovery SoT points Architect at TB-P10-T003-R1; Impl TB-P10-T004-R1; Phase P10", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);
  ...
  assert.match(state, /Last Implementation Task:\s*```text\s*TB-P10-T004-R1\s*```/);
  assert.match(recovery, /Last Implementation Task:\s*```text\s*TB-P10-T004-R1\s*```/);
  ...
  assert.equal(CURRENT_TASK_ID, "TB-P10-T004-R1");
});
...
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(implRecovery, "TB-P10-T004-R1");
  assert.equal(implState, "TB-P10-T004-R1");
