/**
 * Recovery SoT staleness guard (TB-P09-T004).
...
const CURRENT_TASK_ID = "TB-P09-T004";
...
test("recovery SoT points Last Implementation at TB-P09-T004; Architect T003; Issued T004; Repair none", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);

  assert.match(recovery, /TB-P09-T004/);
  assert.match(state, /TB-P09-T004/);
  ...
  assert.match(
    state,
    /Last Implementation Task:\s*```text\s*TB-P09-T004\s*```/,
    "PROJECT-STATE Last Implementation Task must be TB-P09-T004",
  );
  assert.match(
    recovery,
    /Last Implementation Task:\s*```text\s*TB-P09-T004\s*```/,
    "recovery Last Implementation Task must be TB-P09-T004",
  );
  assert.match(
    state,
    /Current Issued Task:\s*```text\s*TB-P09-T004\s*```/,
    "PROJECT-STATE Current Issued Task must be TB-P09-T004",
  );
  assert.match(
    state,
  ...
  assert.match(
    recovery,
    /Current Issued Task:\s*```text\s*TB-P09-T004\s*```/,
    "recovery Current Issued Task must be TB-P09-T004",
  );
  assert.match(
    recovery,
    /Current Repair Task:\s*```text\s*\(none\)\s*```/,
  ...
  assert.equal(CURRENT_TASK_ID, "TB-P09-T004");
  assert.doesNotMatch(state, /TB-P09-T005/);
  assert.doesNotMatch(recovery, /TB-P09-T005/);
});
...
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(implRecovery, "TB-P09-T004");
  assert.equal(implState, "TB-P09-T004");
