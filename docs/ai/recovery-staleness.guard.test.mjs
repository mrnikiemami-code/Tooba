/**
 * Recovery SoT staleness guard (TB-P10-T004-R6).
...
const CURRENT_TASK_ID = "TB-P10-T004-R6";
...
test("recovery SoT points Architect at TB-P10-T004-R5; Impl TB-P10-T004-R6; Phase P10", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);
  assert.match(recovery, /P10/);
  assert.match(state, /P10/);
  assert.match(state, /Last Architect Accepted Task:\s*```text\s*TB-P10-T004-R5\s*```/);
  assert.match(recovery, /Last Architect Accepted Task:\s*```text\s*TB-P10-T004-R5\s*```/);
  assert.match(state, /Last Implementation Task:\s*```text\s*TB-P10-T004-R6\s*```/);
  assert.match(recovery, /Last Implementation Task:\s*```text\s*TB-P10-T004-R6\s*```/);
  ...
  assert.equal(CURRENT_TASK_ID, "TB-P10-T004-R6");
});
...
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(implRecovery, "TB-P10-T004-R6");
  assert.equal(implState, "TB-P10-T004-R6");
