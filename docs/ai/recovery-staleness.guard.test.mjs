/**
 * Recovery SoT staleness guard (TB-P09-T002).
...
const CURRENT_TASK_ID = "TB-P09-T002";
...
test("recovery SoT points Last Implementation at TB-P09-T002; Architect T001; Issued T002; Repair none", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);
  ...
  assert.match(
    state,
    /Last Implementation Task:\s*```text\s*TB-P09-T002\s*```/,
    "PROJECT-STATE Last Implementation Task must be TB-P09-T002",
  );
  assert.match(
    recovery,
    /Last Implementation Task:\s*```text\s*TB-P09-T002\s*```/,
    "recovery Last Implementation Task must be TB-P09-T002",
  );
  assert.match(
    state,
  ...
  assert.equal(CURRENT_TASK_ID, "TB-P09-T002");
...
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(implRecovery, "TB-P09-T002");
  assert.equal(implState, "TB-P09-T002");
