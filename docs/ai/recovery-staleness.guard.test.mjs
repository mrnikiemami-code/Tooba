/**
 * Recovery SoT staleness guard (TB-P09-T006).
...
const CURRENT_TASK_ID = "TB-P09-T006";
...
test("recovery SoT points Last Implementation at TB-P09-T006; Architect T005; Issued none; Repair none", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);

  assert.match(recovery, /TB-P09-T006/);
  assert.match(state, /TB-P09-T006/);
  ...
  assert.match(
    state,
    /Last Implementation Task:\s*```text\s*TB-P09-T006\s*```/,
    "PROJECT-STATE Last Implementation Task must be TB-P09-T006",
  );
  assert.match(
    recovery,
    /Last Implementation Task:\s*```text\s*TB-P09-T006\s*```/,
    "recovery Last Implementation Task must be TB-P09-T006",
  );
  assert.match(
    state,
  ...
  assert.equal(CURRENT_TASK_ID, "TB-P09-T006");
...
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(implRecovery, "TB-P09-T006");
  assert.equal(implState, "TB-P09-T006");
