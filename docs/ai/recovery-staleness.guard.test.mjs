/**
 * Recovery SoT staleness guard (TB-P09-T005-R1).
...
const CURRENT_TASK_ID = "TB-P09-T005-R1";
...
test("recovery SoT points Last Implementation at TB-P09-T005-R1; Architect T004; Issued none; Repair R1", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);

  assert.match(recovery, /TB-P09-T005-R1/);
  assert.match(state, /TB-P09-T005-R1/);
  assert.match(recovery, /P09/);
  assert.match(state, /P09/);

  assert.match(
    state,
    /Last Architect Accepted Task:\s*```text\s*TB-P09-T004\s*```/,
    "PROJECT-STATE Last Architect Accepted Task must be TB-P09-T004",
  );
  assert.match(
    state,
    /Last Architect-Accepted Task:\s*```text\s*TB-P09-T004\s*```/,
  ...
  assert.match(
    recovery,
    /Last Architect Accepted Task:\s*```text\s*TB-P09-T004\s*```/,
    "recovery Last Architect Accepted Task must be TB-P09-T004",
  );
  assert.match(
    state,
    /Last Implementation Task:\s*```text\s*TB-P09-T005-R1\s*```/,
    "PROJECT-STATE Last Implementation Task must be TB-P09-T005-R1",
  );
  assert.match(
    recovery,
    /Last Implementation Task:\s*```text\s*TB-P09-T005-R1\s*```/,
    "recovery Last Implementation Task must be TB-P09-T005-R1",
  );
  assert.match(
    state,
  ...
  assert.match(
    state,
    /Current Repair Task:\s*```text\s*TB-P09-T005-R1\s*```/,
    "PROJECT-STATE Current Repair Task must be TB-P09-T005-R1",
  );
  assert.match(
    recovery,
    /Current Issued Task:\s*```text\s*\(none\)\s*```/,
  ...
  assert.match(
    recovery,
    /Current Repair Task:\s*```text\s*TB-P09-T005-R1\s*```/,
    "recovery Current Repair Task must be TB-P09-T005-R1",
  );
  assert.match(state, /USER_VISUAL_ACCEPTED:\s*```text\s*NO\s*```/);
  assert.match(recovery, /USER_VISUAL_ACCEPTED:\s*```text\s*NO\s*```/);
  assert.equal(CURRENT_TASK_ID, "TB-P09-T005-R1");
  assert.doesNotMatch(state, /TB-P09-T006/);
  assert.doesNotMatch(recovery, /TB-P09-T006/);
});
...
test("recovery SoT does not leave stale tasks as Last Implementation", () => {
  ...
  assert.equal(implRecovery, "TB-P09-T005-R1");
  assert.equal(implState, "TB-P09-T005-R1");
