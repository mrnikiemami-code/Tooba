/**
 * Recovery SoT staleness guard (TB-P08-T001-R1).
...
const CURRENT_TASK_ID = "TB-P08-T001-R1";
...
test("recovery SoT current issued/repair points at idle after TB-P08-T001-R1 not stale pointers", () => {
  const recovery = read(recoveryPath);
  const state = read(statePath);

  assert.match(recovery, /TB-P08-T001-R1/);
  assert.match(state, /TB-P08-T001-R1/);
