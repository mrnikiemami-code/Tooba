...
/**
 * پیشخوان Shopeiva با دادهٔ زنده؛ نمودار بازدید/فروش و امتیاز جعلی حذف شده است.
 */
export default function CustomerDashboard() {
  ...
  return (
    <div className="space-y-6" data-testid="customer-dashboard">
      ...
      <section className="grid grid-cols-2 xl:grid-cols-4 gap-3" data-testid="customer-dashboard-stats">
        <Metric icon={Package} value={page.totalOrders} label="کل سفارشات" tone="blue" />
        <Metric icon={WalletCards} value={page.paidOrders} label="پرداخت‌شده" tone="green" />
        <Metric icon={Eye} value={page.pendingOrders} label="در انتظار پرداخت" tone="amber" />
        <Metric
          icon={Heart}
          value={page.wishlistCount}
          label="علاقه‌مندی‌ها"
          tone="pink"
          suffix={page.wishlistAvailable ? undefined : "غیرفعال"}
        />
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <section className="bg-white rounded-2xl border border-gray-200 p-4 md:p-6 shadow-sm space-y-4">
          ...
          <SummaryRow
            label="آدرس‌ها"
            value={
              page.addressBookAvailable
                ? `${page.addressBookCount.toLocaleString("fa-IR")} مورد`
                : "غیرفعال"
            }
          ...
          <SummaryRow
            label="علاقه‌مندی"
            value={
              page.wishlistAvailable ? `${page.wishlistCount.toLocaleString("fa-IR")} کالا` : "غیرفعال"
            }
...
function SummaryRow({ label, value, href }: { label: string; value: string; href: string }) {
  return (
    <Link href={href} className="flex items-center justify-between gap-2 rounded-xl border border-gray-100 px-3 py-3 hover:border-blue-200">
