...
function storyStatusBadge(status: string): string {
...
/** مدیریت استوری‌های ویترین با DataGrid و فرمان‌های Host. */
export function AdminStoriesScreen() {
  ...
  const refresh = useCallback(() => void listAdminStories().then((result) => {
    setState(result.state);
    setRows(result.data ?? []);
    setMessage(result.message);
  }), []);
  ...
  const columns = useMemo((): GridColumnDef<AdminStorySnapshot>[] => [
    ...
    },
    {
      ...
      accessor: (row) => row.status,
      cell: (row) => (
        <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${storyStatusBadge(row.status)}`}>
          {STORY_STATUS_LABELS[row.status] ?? row.status}
        </span>
      ),
      width: 110,
      minWidth: 90,
  ...
  return (
    <main data-testid="admin-stories">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <PageHeading title="استوری‌ها" description="ایجاد، فعال‌سازی و آیتم‌های Story ویترین" />
      ...
      <section className="overflow-hidden rounded-2xl border border-border bg-surface-elevated shadow-sm">
        <div className="border-b border-border px-5 py-3 text-sm text-muted">{rows.length.toLocaleString("fa-IR")} استوری</div>
        <div className="p-2 md:p-4">
          {state === "error" ? (
            <ErrorState title="استوری‌ها خوانده نشد" detail={message} onRetry={refresh} retryLabel={faWorkspaceMessages.retry} />
          ) : (
            <DataGrid columns={columns} queryAdapter={queryAdapter} />
          )}
        </div>
      ...
      {selected ? (
        <section className="mt-5 rounded-2xl border border-border bg-surface-elevated p-5 shadow-sm">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button type="button" className="rounded-xl px-3 py-1.5 text-sm" onClick={() => setSelectedId(null)}>بستن</button>
          </div>

          <div className="mb-4 grid gap-3 md:grid-cols-2">
            <label className="block text-sm">
              <span className="mb-1 block text-gray-600">شروع (اختیاری)</span>
              <input
                type="datetime-local"
                className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-gray-600">پایان (اختیاری)</span>
              <input
                type="datetime-local"
                className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
          ...
          <button
            ...
            onClick={() => void scheduleAdminStory(selected.storyId, {
          ...
          <h3 className="mb-2 text-sm font-bold">آیتم‌ها ({selected.items.length.toLocaleString("fa-IR")})</h3>
          ...
          <button
            ...
            onClick={() => void addAdminStoryItem(selected.storyId, itemDraft).then((result) => {
              if (result.state === "ok") refresh();
              else setMessage(result.message);
            })}
          >
      ...
      {showCreate ? (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-5 shadow-xl">
            <h2 className="mb-4 text-lg font-bold">ایجاد استوری</h2>
            <div className="space-y-3">
              {([
                ["title", "عنوان"],
                ["locale", "locale"],
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                type="button"
                className="rounded-xl bg-[#2563EB] px-4 py-2 text-sm font-bold text-white"
                onClick={() => void createAdminStory(draft).then(async (result) => {
