...
public sealed record ArchiveOfferCommand(Guid OfferId, Guid SellerPartyId) : IRequest<OfferReference>;

internal sealed class ArchiveOfferHandler(IOfferStore store, IClock clock)
    : IRequestHandler<ArchiveOfferCommand, OfferReference>
{
    public async Task<OfferReference> Handle(ArchiveOfferCommand request, CancellationToken cancellationToken)
    {
