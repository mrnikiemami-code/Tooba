...
public sealed class SettlementDirectory : ISettlementDirectory
{
    ...
    {
        await _guard.EnsureCanMutateAsync(cancellationToken);
        return await ExecutePayoutAsync(command.PayoutRequestId, $"retry:{command.ActorUserId:N}:{_clock.UtcNow.Ticks}", cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var now = _clock.UtcNow;
        var attempt = request.BeginAttempt(_ids.NewId(), attemptIdempotencyKey, now);
        ...
        CancellationToken cancellationToken)
    {
        ...
        account = SettlementAccount.Create(_ids.NewId(), sellerPartyId, currency, now);
        ...
        {
            _db.SellerPayoutProfiles.Add(SellerPayoutProfile.CreateDevPlaceholder(_ids.NewId(), sellerPartyId, now));
        }
    ...
    private async Task<CommissionPolicy> GetDefaultCommissionPolicyAsync(CancellationToken cancellationToken)
    {
        ...
        policy = CommissionPolicy.CreateDefaultMarketplace(_clock.UtcNow);
