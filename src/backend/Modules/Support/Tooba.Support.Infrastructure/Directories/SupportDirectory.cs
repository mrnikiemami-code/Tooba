...
public sealed class SupportDirectory : ISupportDirectory
{
    ...
    public SupportDirectory(SupportDbContext db, INotificationCreationPort notifications, IClock clock, IIdGenerator ids)
    {
        _db = db;
    ...
    public Task<TicketSnapshotDto> CloseForCustomerAsync(
        ...
        MutateOwnedAsync(
            t => t.TicketId == ticketId
                 && t.RequesterKind == RequesterKind.Customer
                 && t.RequesterActorUserId == actorUserId,
            ticket => ticket.CloseByRequester(_clock.UtcNow),
            cancellationToken);
    ...
    public Task<TicketSnapshotDto> ReopenForCustomerAsync(
        ...
        MutateOwnedAsync(
            t => t.TicketId == ticketId
                 && t.RequesterKind == RequesterKind.Customer
                 && t.RequesterActorUserId == actorUserId,
            ticket => ticket.ReopenByRequester(_clock.UtcNow),
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        ticket.ApplyAdminPatch(status, priority, command.AssignedOperatorActorUserId, clearAssignee, _clock.UtcNow);
        ...
        if (ticket.RequesterKind == RequesterKind.Customer)
        {
            await _notifications.CreateIfAbsentAsync(
                new CreateNotificationCommand(
                    ...
                    ticket.RequesterActorUserId,
                    NotificationSemanticTypes.SupportAdminReply,
