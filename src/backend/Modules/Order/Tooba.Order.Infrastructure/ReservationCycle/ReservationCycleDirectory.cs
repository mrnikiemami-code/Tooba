...
public sealed class ReservationCycleDirectory : IReservationCycleDirectory
{
    ...
    /// <inheritdoc />
    public ReservationCycleEntity PrepareStart(
        Guid checkoutId,
        ...
        Guid? paymentAttemptId)
    {
        ...
        var cycle = ReservationCycleEntity.Start(
            checkoutId,
            next,
            reason,
            startedAt,
    ...
    private static ReservationCycleProjection ProjectRows(
        Guid checkoutId,
        IReadOnlyList<ReservationCycleEntity> history,
        DateTimeOffset serverNow,
        string? supplyStatus)
    {
    ...
    private async Task<ReservationCycleEntity?> LoadActiveAsync(Guid checkoutId, CancellationToken cancellationToken) =>
        await _db.ReservationCycles
            .Where(x => x.CheckoutId == checkoutId && x.Status == ReservationCycleStatus.Active)
            .OrderByDescending(x => x.CycleNumber)
            .FirstOrDefaultAsync(cancellationToken);

    private static ReservationCycleSnapshot ToSnapshot(ReservationCycleEntity cycle) =>
        new(
            cycle.CycleId,
            cycle.CheckoutId,
            cycle.CycleNumber,
