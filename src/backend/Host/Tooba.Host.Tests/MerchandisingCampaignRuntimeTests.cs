...
public sealed class MerchandisingCampaignRuntimeTests : IAsyncLifetime
{
    ...
    public async Task Merchandising_campaign_runtime_selection_availability_locale_price_and_seed()
    {
        ...
        var merchDir = new MerchandisingCampaignDirectory(promoDb, new SystemUtcClock(), new UuidV7IdGenerator());
        var query = new MerchandisingCampaignQuery(promoDb, offerDir, priceDir, inventoryDir, new ModuleCallTracer());
